...
void LinkedQueues :: insert(int element) /* Inserts node in the queue */
{
    ...
    {
        /* If the queue is NOT empty, then REAR points to the last node. REAR should now
...
void LinkedQueues :: remove() /* deletes node from the queue */
{
    ...
    else
    {
        cout<<"\nThe element deleted from the queue is: "<<
            FRONT->data<<endl;
        FRONT = FRONT->next; /* FRONT will now point to the next node in the
queue. */
    }
}
...
{
    LinkedQueues q;
http://www.niitstudent.com/India/Content/133DSALGL1/OEBPS/Proxy.aspx?Q=chap05.htm[09/05/2014 10:59:19 AM]
    char ch;
    int num;
    do
    {
